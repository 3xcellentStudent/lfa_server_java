...
public class CanadaPostService {
  ...
  @Value("${delivery.canadapost.create_shipment}")
  private String CREATE_SHIPMENT_ROUTE;
  @Value("${delivery.canadapost.pickup_availability}")
  private String PICKUP_AVAILABILITY_ROUTE;
  @Value("${delivery.canadapost.ship_price}")
  ...
  public ResponseEntity<Object> createShipment(){
    ...
    try {
      HttpRequest request = HttpRequest.newBuilder()
      .uri(new URI(CREATE_SHIPMENT_ROUTE))
